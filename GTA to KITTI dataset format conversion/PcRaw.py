...
class PcRaw:
    ...
    def generatePointCloudWithoutBackground(self, category_id_list, debug_mode = False):
        '''
        ...
        points = []
        for i in range(0, len(self.list_rotated_raw_pc)):
            notBackgroundPoint = False
            for id in category_id_list:
                if self.list_raw_labels[i] == id:
                    notBackgroundPoint = True
                    break
            
            if notBackgroundPoint:
                points.append(self.list_rotated_raw_pc[i])
