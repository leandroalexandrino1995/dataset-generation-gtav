...
class GtaSample:
    ...
    def __init__(self, sampleDirPath, configurations):
        '''
        Constructor
        Arguments:
        ...
        rotationFIleLine = self.loadTxtFileIntoStrList(configurations.rotationFn)
        ...
        # -camRot makes the point cloud face the y direction in the right handed coord system, and -90 makes the point cloud face de x direction
        # get Z rotation of the camera (character) stored in file
        self.camRotation = - (self.rawCamRotation) - 90

        self.imageView = GtaView(sampleDirPath, configurations.fvImgFn)

        #### Core calculations over the point cloud ####
        originalPc = None
        if configurations.useNoise:
            # use pointcloud with error
            originalPc = self.loadPlyFileIntoTupleList(configurations.pcPlyNoiseFn)
        else:
            # use perfect pointcloud
            # tuple list with all the points of the point cloud, each point is a tuple (x, y, z)
            originalPc = self.loadPlyFileIntoTupleList(configurations.pcPlyFn)

        # load list of integers with the labels (background (0), pedestrian (1), vehicle (2), game props (3))
        pointLabels = self.loadTxtFileIntoIntList(configurations.pcLabelsFn)
